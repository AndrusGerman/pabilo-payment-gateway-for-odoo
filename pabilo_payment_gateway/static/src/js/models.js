odoo.define('pabilo_payment_gateway.models', function (require) {
    ...
    const { register_payment_method, Payment, Order } = require('point_of_sale.models');
     ...
     * cajero no puede cobrar. El constructor sí tiene esa guarda («Please
     * configure a payment method in your POS»); la ruta de restauración no.
     *
     * Con un método de pago por cuenta bancaria, ese conjunto cambia más que
     * antes: se crean, se renombran y se archivan cuando la cuenta desaparece de
     * Pabilo. Que un cambio de configuración pueda dejar una caja sin abrir no es
     * aceptable, así que se descarta la línea huérfana y el pedido se restaura
     * sin ella: queda con su saldo pendiente, que es recuperable y evidente.
     *
     * La guarda es genérica, no solo para Pabilo, porque el método que falta ya
     * no existe y no hay forma de saber de quién era.
     */
    Registries.Model.extend(Order, (Order) =>
        class extends Order {
            init_from_JSON(json) {
